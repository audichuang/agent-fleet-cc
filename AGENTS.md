...
A Claude Code plugin marketplace: 4 plugins (`codex`, `antigravity`, `delegate`,
...
(`plugins/{codex,antigravity,delegate}/`, `tests/{codex,antigravity,delegate}/`).
...
## Commands
- `npm test` — full chain: structure + shared + delegate + antigravity + codex + fleet (Node >= 22.3)
- `node --test tests/<plugin>/*.test.mjs` — run one plugin's suite
- `npm run test:e2e` — delegate black-box CLI e2e (real subprocess, fake engine, no API key)
- `npm run sync-shared` — after editing `shared/lib/`, re-vendor it into each migrated
  plugin's `scripts/lib/shared/` (currently only `delegate`). **Commit BOTH the source and
...
- `shared/lib/` is the source of truth. `delegate` (v0.2.0) is migrated onto it;
