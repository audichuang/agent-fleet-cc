...
### Gap 2 — progress updates (the B3 race source) — **RESOLVED = OPTION A (Codex-gated 2026-06-29)**

> **SETTLED.** Decision + no-clobber proof + adversarial-test design: `2026-06-29-codex-gap2-b3-resolution.md`. Resolution = **Option A** (progress → `events.ndjson` via the existing `engine-event` type; `job.json` written only by `markJobRunning` + `finalizeJob`). Lock-free Option B proven impossible. The Option A/B fork text below is retained for the record; implement per the resolution doc and `1c` above (with gate must-fixes R1–R5).
...
- [ ] **1c — Terminal CAS facade + Gap 2 resolution = OPTION A (SETTLED + Codex-gated 2026-06-29; see `2026-06-29-codex-gap2-b3-resolution.md`).** `claimTerminalTransition` recreate fallbacks route through PUBLIC `finalizeJob` (shared `claimTerminalTransition` is private). Terminal patch → shared `finalizeJob`. **Progress patch (threadId/turnId/phase) → `appendEvent(jobDir,"engine-event",…)` into `events.ndjson`, NOT `job.json`** (the old "readJob + active-gate + writeJob" variant is WRONG — it reopens B3: shared `writeJob` is unconditional/lockless. Do NOT implement it.). This makes B3 (progress clobber) structurally impossible. **Cutover MUST be atomic** — progress→events lands in the SAME change that routes terminal→finalizeJob, never half-and-half (routing terminal to `finalizeJob` while progress still touches `job.json` under `.wlock` serializes the two on different files = reopened B3). Lock-free Option B is impossible (gated). **Gate must-fixes (R1–R5):** R1 status/list readers become terminal-lock-aware (a `markJobRunning`-vs-`finalizeJob` race leaves a documented stale `running`; lock is authoritative — `worker.test.mjs:289-294`); R3 repoint `/codex:cancel` (`codex-companion.mjs:1169-1171→:1220`) to the events overlay; R4 one `readCurrentTurnIdentity` helper recovering BOTH ids independently for all four correctness readers (watchdog/cancel/crash-net/timeout). The B-suite race tests (apply-job-patch, tracked-jobs-race, session-cleanup-cas-order) must still pass; add the §5 deterministic fails-first test (no-hook sequenced) + the markRunning-vs-finalize test.
