...
- **thread/start settings:** `{ approvalPolicy:"never", sandbox: resolveSandboxMode() }`
  ...
  - **The hard `-32600` "you need to use auto review" is a property of the ROUTE, not of one
    field — corrected 2026-08-23 (third pass).** This doc previously said it fires *only* on an
    explicit `approvals_reviewer = User`. **That is false.** `:232-239` sends a thread-settings
    update carrying nothing but `sandbox_policy = Some(SandboxPolicy::DangerFullAccess)` on a
    protected-model thread and asserts the same rejection (`assert_error` pins the code at `:84`).
    The claim was disproven by bytes inside the very range the old wording cited (`:202-239`). The
    proven matrix, per route:
    - **`thread/start` — coerces everything; there is no reject case.** `approval_policy = Never`
      + `approvals_reviewer = User` + `sandbox = DangerFullAccess` in one call **succeeds**, with
      the sandbox downgraded to `WorkspaceWrite` (`:162-180`).
    - **`thread/settings/update` — rejects two *different* fields.** `approvals_reviewer = User`
      (`:210-220`, the `UNSAFE` loop) **and** a lone `sandbox_policy = DangerFullAccess`
      (`:232-239`). It **accepts and coerces** any explicitly-requested `approval_policy`
      (`:221-231`).
    - **`turn/start` override — rejects `approvals_reviewer = Some(User)`**, both on a thread
      already on the protected model (`:240-247`) and with `model` set in the same turn
      (`:250-258`). A turn carrying just `thread_id` + a protected `model` **succeeds** and
      coerces via a settings-updated notification (`:259-267`, and with an explicit thread policy
      `:269-286`).
    **The plugin's own conclusion survives the correction, on a narrower argument.** The reject
    lane belongs to the two *mutate-an-existing-thread* routes, and the plugin sends **neither**
    trigger field on either: its `turn/start` params are
    `{threadId, input, model|null, effort|null, outputSchema|null}` (see the params bullet above),
    and `grep -rn "settings/update\|sandboxPolicy\|thread/fork" plugins/codex/scripts`
    (excluding the vendored `lib/shared/`) is **empty** — the plugin never sends a settings update
    at all. Keep it that way, and note the one **untested cell**: `TurnStartParams` *does* carry an
    optional `sandboxPolicy` (`app-server-protocol/schema/json/v2/TurnStartParams.json`, top-level
    `properties`; only `input` + `threadId` are required), and **no upstream test sends it on a
    protected model**. Given `:232-239`, that is the single field never to add to our `turn/start`.
  ...
  - **`thread/resume` / `thread/fork` are on the coerce lane, PROVEN for the approval fields:**
    ...
    `assert_protected_with_policy(… Never)` every time — never `assert_error`.
    `buildResumeParams` (`codex.mjs:97-105`) sends `approvalPolicy:"never"` and **no**
    `approvalsReviewer` at all.
  - **What the resume conclusion now rests on — restated 2026-08-23 (third pass).** It used to
    rest on "the reviewer field is the only one that ever triggers `-32600`"; the bullet above
    killed that premise, so the argument is rebuilt from what is actually proven:
    - **Discriminating evidence, PROVEN:** resume and fork **accept** `approvals_reviewer =
      Some(User)` (`:355-369`, `:325-340`) — the exact field `thread/settings/update` hard-rejects
      at `:210-220`. So `thread/resume` behaves like the settings-**establishment** lane
      (`thread/start`), not the settings-**mutation** lane that owns the reject case. That is the
      load-bearing argument now, and it is sharper than the premise it replaces: it distinguishes
      the two lanes instead of assuming there is only one trigger field.
    - **Still INFERENCE, and it matters more than it used to:** no resume test sends a `sandbox`
      field, while `buildResumeParams` sends `danger-full-access`. `:162-180` remains the *only*
      anchor for the `danger-full-access` → `workspace-write` downgrade and it is a `thread/start`
      anchor. Sandbox is no longer a field that is harmless everywhere — one lane coerces it
      (`:162-180`), another rejects it (`:232-239`) — so "resume coerces our sandbox" is an
      inference that resume follows the lane the bullet above places it on, not a proof.
    - **Residual failure mode if that inference is wrong:** on a managed workspace with a
      protected model, `thread/resume` would return `-32600` — a **rejected `client.request`**
      (`codex.mjs:1111`), i.e. a loud JSON-RPC request error, not a silent run with the wrong
      sandbox. Note it would *not* travel the `TurnError` path: `describeTurnError` only ever sees
      `result.turn?.error` / `result.error` (`codex.mjs:1255-1256`), so the surfaced wording would
      come from the request rejection, not from that formatter. Cheap proof either way is a resume
      test carrying a sandbox field; upstream has not written one.
...
|---|---|---|---|
| 2026-08-23 (third pass) | `99660ab3c7` (main; unchanged) | 1.6.0 (**unchanged** — doc-only, and this time the word is accurate) | **A second independent Codex (gpt-5.6) review found the second pass's `-32600` scope claim FALSE, and the host re-verified it by reading the bytes.** The doc said the managed-auto-review rejection fires *only* on an explicit `approvals_reviewer = User`; `model_auto_review.rs:232-239` sends a **thread-settings update carrying only `sandbox_policy = Some(SandboxPolicy::DangerFullAccess)`** and asserts the same `-32600` (`assert_error`, code pinned `:84`) — bytes that sat *inside* the `:202-239` range the old wording cited. **Three fixes, all doc:** (1) the checklist bullet is rewritten **per route, not per field** — `thread/start` coerces everything incl. sandbox (`:162-180`); `thread/settings/update` rejects **two** fields, `approvals_reviewer = User` (`:210-220`) *and* a lone `sandbox_policy` (`:232-239`), while coercing any explicit `approval_policy` (`:221-231`); the `turn/start` override rejects `approvals_reviewer` (`:240-258`) but coerces a bare `thread_id` + protected `model` (`:259-267`, `:269-286`). The plugin's conclusion **survives on a narrower argument**: the reject lane belongs to the two mutate-an-existing-thread routes and the plugin sends neither trigger field on either — `grep -rn "settings/update\|sandboxPolicy\|thread/fork" plugins/codex/scripts` (minus vendored `lib/shared/`) is **empty**. New **untested cell** recorded: `TurnStartParams` does carry an optional `sandboxPolicy` (`schema/json/v2/TurnStartParams.json`, only `input` + `threadId` required) and no upstream test sends it on a protected model — so that is the one field never to add to our `turn/start`. (2) The **resume argument is rebuilt**, because it had leaned on the dead premise. Its load-bearing anchor is now *discriminating* rather than absolute: resume/fork **accept** `approvals_reviewer = Some(User)` (`:325-340`, `:355-369`) — the exact field the settings-update lane hard-rejects — which places resume on the establishment lane with `thread/start`. The sandbox half stays **inference** and now matters more, since sandbox is a proven reject trigger on one lane and a proven coerce target on another; residual failure mode if wrong is a **loud** `-32600` on a managed-workspace resume, not a silent wrong-sandbox run. (3) The second-pass row is **annotated, not rewritten**, in three places — including its own “doc-only … touched no plugin code”, which is true of the protocol-audit subpass but not of the commit that carried it (`4b982c9` also changed `codex-companion.mjs`, `lib/codex.mjs`, `lib/job-control.mjs`, `lib/render.mjs`; confirmed with `git show --stat`). **Re-proven this pass** (opened at `99660ab3c7`): `app-server/tests/suite/v2/model_auto_review.rs` **read end-to-end, all 400 lines** — every anchor in the checklist plus the previously-unread `:232-239`; `v2/TurnStartParams.json`'s property list; `buildResumeParams` / `buildThreadParams` / `resolveSandboxMode` (`codex.mjs:70-105`); the plugin's absence of any settings-update or `sandboxPolicy` send; and `git show --stat` on `4b982c9` / `8a94501`. **Carried forward, NOT re-run:** the first pass's whole wire-schema sweep, the notification / item-variant / server-request checklist assertions, `build:codex`, and the live-engine smoke. **Lesson for the next pass:** the second pass cited a line *range* and reasoned only from the part it had read — an anchor is not verified until the whole range is opened. |
| 2026-08-23 (second pass) | `99660ab3c7` (main; unchanged) | 1.5.0 → **1.6.0** | **Doc-only correction pass — no new upstream diff was run.** *(Annotation, 2026-08-23 third pass: “doc-only” is true of the **protocol-audit subpass only** — the commit this row describes, `4b982c9`, also carried the codex runtime fixes it lists elsewhere: `scripts/codex-companion.mjs`, `scripts/lib/codex.mjs`, `scripts/lib/job-control.mjs`, `scripts/lib/render.mjs` + 4 test files. Verdict unchanged; the scope word was too wide.)* An independent Codex (gpt-5.6) review of the first pass's commits found this doc's managed-auto-review anchor **overstating what its test proves**, and the host re-verified it by reading the bytes. Two fixes: (1) **Baseline** said `codex@1.5.0`; the repo is `1.6.0` (`npm run check-version`) since the failed-turn fix landed. (2) The durable checklist's `thread/start` / `thread/resume` bullet is rewritten to separate proof from inference, all anchors re-read at `99660ab3c7`: the resume/fork test (`model_auto_review.rs:290-381`) sends `approvals_reviewer = Some(User)` and **no `sandbox` field**, while the plugin sends the reviewer *not at all* and `danger-full-access` — so "the same unsafe pair the plugin sends" was wrong in both directions. The coerce-lane conclusion **survives** (and is stronger than before: `-32600` fires only on an explicit `approvals_reviewer`, `:202-239` / `:240-258`, and a bare `turn/start` with a protected `model` coerces fine at `:259-267` — exactly the plugin's shape), but the **sandbox** half of it is now labelled inference, since `:162-180` is a `thread/start` anchor. *(Annotation, 2026-08-23 third pass: “`-32600` fires only on an explicit `approvals_reviewer`” is **FALSE** — `:232-239`, inside the range this row itself cites, rejects a settings update carrying only `sandbox_policy = DangerFullAccess`. The row's conclusion about the plugin's own `turn/start` shape still holds; see the third-pass row below and the rewritten checklist bullet for the per-route matrix that replaces the per-field claim.)* Same pass also corrected a claim this doc had carried since 2026-08-09: the coerced approval policy is **not** always `on-request` — an explicitly-requested policy survives (`:36-47`, `:132-146`), and the plugin sends `never` explicitly, so its coerced triple is `never` + `auto_review` + `workspace-write`. **Re-proven this pass:** every `model_auto_review.rs` anchor in the checklist, plus `buildResumeParams` (`codex.mjs:97-105`) and `resolveSandboxMode` (`:70-82`). **Carried forward, not re-run:** the whole 2026-08-23 first-pass wire-schema sweep and `build:codex` — this pass touched no plugin code. *(Annotation, 2026-08-23 third pass: that last clause is wrong as written and right as meant — the **protocol-audit subpass** touched no plugin code, but its commit `4b982c9` did; `build:codex` was genuinely not re-run either way, so the carried-forward list stands.)* |
