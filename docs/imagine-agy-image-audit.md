...
| Decision | Because |
|----------|---------|
| Renders into a private staging dir, judges it by `statSync` + magic bytes, then publishes with `COPYFILE_EXCL` | It is an agent, so `status: SUCCESS` proves nothing — the reason `/grok:image` was retired. The extension is only knowable after the bytes exist, so correcting it in place would rename over a path the pre-flight check never saw; an `O_EXCL` create cannot clobber and cannot be raced. |
...
| Sends `--dangerously-skip-permissions`, `detached`, with `cwd` set to the staging dir | A headless run cannot answer a permission prompt. `detached` gives agy its own process group so a timeout can signal the whole tree — signalling only the leader leaves helpers holding the pipes and `close` never arrives (measured: a 100ms timeout settling at 1087ms). **`cwd` is not a fence** — an agent with permissions skipped reaches anywhere by absolute path; cwd only decides where a *relative* path of agy's lands. |
| On timeout: SIGTERM → SIGKILL → settle on our own deadline, and keep staging if the tree was never confirmed dead | Even SIGKILL cannot force `close` when a descendant holds the pipes, so the promise needs its own bound. And a directory a live renderer may still be writing into must not be deleted — that would destroy a render the user paid for. |
