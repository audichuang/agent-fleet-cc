...
- **Grok Imagine / `image_gen` — WIRED as of 2026-08-23 (`/grok:image`).** Not a companion verb:
  ...
  - **Saved-file format is a naming convention, not a guarantee.**
    ...
    **warning only**.
  - **Two claims are INFERENCE, not yet observed against the real binary — confirm on first
    live run:** (1) that a tier-restricted run exits 0 with no error event, and (2) that the
    upsell text reaches the raw log we read back with `logs <job>`.
