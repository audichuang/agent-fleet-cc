...
| What | Value |
| --- | --- |
| Plugin | `plugins/grok/` @ `0.8.0` (`npm run check-version`) — the 2026-08-23 first-pass fixes shipped in `0.7.0`, the second-pass corrections on top of it, and `0.8.0` **removed `/grok:image`** (generation moved to the `imagine` plugin; see the fourth-pass row) |
...
**One entry graduated and was then UN-wired the same day:** Grok Imagine (`image_gen`) was wired
by `/grok:image` on 2026-08-23, and `/grok:image` was **removed in `grok@0.8.0`** hours later.
Image generation moved to the separate `imagine` plugin, which calls xAI's
`POST /v1/images/generations` **directly** and never involves this CLI — see
`docs/imagine-xai-image-api-audit.md`. **Why:** the companion path could only learn whether an
image existed by reading a job's raw event stream, and a tier block arrives there as a
*successful* tool result — failure that looks like success. An HTTP call returns bytes or a
status. The bullet below is **SUPERSEDED for the plugin** but every fact in it is still true *of
the engine*, so it stays: nothing plugin-side depends on it today, and a future pass that
considers routing back through the CLI should not have to re-discover it.

- **[SUPERSEDED 2026-08-23 — the plugin no longer uses this path] Grok Imagine / `image_gen`,
  as wired by the former `/grok:image`.** Not a companion verb:
...
| 2026-08-23 (fourth pass) | live `api.x.ai` with the grok CLI's own OAuth bearer; no new engine diff | **surface abandoned, not broken** | `/grok:image` **removed** (`grok@0.8.0`). Not a contract failure — `image_gen` works, and the second pass's live run proves it. It is an *observability* verdict: the only success signal the companion path offers is prose in a raw event stream, and the tier-restricted branch returns its upsell as a **successful** tool result, so the plugin's gate had to be "a file appeared on disk" plus a substring hunt that the second pass already had to narrow once (it was matching the user's own prompt text). Generation moved to the new `imagine` plugin over `POST /v1/images/generations`, where the failure signal is an HTTP status. **New evidence gathered while doing it** (recorded in full in `docs/imagine-xai-image-api-audit.md`, not here): the grok CLI's access token carries the **`api:access`** scope and a **6 h** lifetime, and is accepted by `api.x.ai` — one live generation returned a 273,397-byte JPEG with no `XAI_API_KEY` present. That makes grok's `auth.json` a **read-only dependency of a different plugin**; `.refresh_token` is deliberately never touched, since auth.x.ai may rotate it on use and that would log the user out of grok. Part 4's Imagine bullet is kept and marked superseded. **Nothing in Parts 1-3 or 5 was re-run this pass.** |
