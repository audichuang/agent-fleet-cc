...
Eight jobs a CLI image plugin actually gets asked for. Each is runnable as-is: the prompt text goes
in quotes, the aspect and model go on the flags.
...
- **The prompt goes on stdin via a quoted heredoc.** Several of these contain double quotes
  around on-image text, and passing them as a shell argument strips the quotes silently.
...
```bash
node …/imagine.mjs --out /abs/path/image.jpg --aspect 1:1 --model grok-imagine-image-2.0 <<'PROMPT'
...
```bash
node …/imagine.mjs --out /abs/path/image.jpg --aspect 3:4 --model grok-imagine-image-2.0 <<'PROMPT'
...
```bash
node …/imagine.mjs --out /abs/path/image.jpg --aspect 2:3 --model grok-imagine-image-2.0 <<'PROMPT'
...
```bash
node …/imagine.mjs --out /abs/path/image.jpg --aspect 16:9 --model grok-imagine-image-2.0 <<'PROMPT'
...
```bash
node …/imagine.mjs --out /abs/path/image.jpg --aspect 4:3 --model grok-imagine-image-2.0 <<'PROMPT'
...
```bash
node …/imagine.mjs --out /abs/path/image.jpg --aspect 3:2 --model grok-imagine-image-2.0 <<'PROMPT'
