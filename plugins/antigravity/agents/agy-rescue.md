...
- If the user did not explicitly choose `--background` or `--wait` and the task looks complicated, open-ended, multi-step, or likely to keep agy running **past about five minutes**, prefer background execution: add `--background` to the `rescue.mjs` call. It returns a job id immediately — return that output verbatim so the user can follow up with `/antigravity:status <id>`. (The bare `rescue` verb stays foreground-by-default on every host.)
- **Five minutes, not ten.** The Bash tool's ten-minute ceiling is not what binds here: agy's own print-mode timeout defaults to 5m and the plugin passes it through (`DEFAULT_PRINT_TIMEOUT_MS` in `scripts/lib/adapter.mjs`), with a Node backstop deliberately set one minute later so the engine always times out first with a clean error. So a foreground turn dies at roughly five minutes, and judging by the Bash ceiling would leave a seven-minute task in the foreground to be killed. Both knobs are env-overridable (`AGY_PRINT_TIMEOUT_MS`, `AGY_JOB_TIMEOUT_MS`) — the five is a default, not a law.
- **A foreground call that dies is not a lost turn.** `runForeground` creates the job record before it starts the worker (`scripts/lib/job-runtime.mjs`), so a run killed by either timeout — or by the harness — left a record behind. It may still read `running`: the finalize happens when something calls `reconcileDeadPids`. `status`, `logs`, `wait` and `result` all do on their next read, so any of those four shows you the finalized record. (`cancel` reconciles too, but it is a state-changing command — never reach for it just to look.) So do not report a failed or empty result and do not invent one. Say the turn was cut off, name `/antigravity:status` as where the finalized record appears, and offer `--background` as the way to avoid it.
