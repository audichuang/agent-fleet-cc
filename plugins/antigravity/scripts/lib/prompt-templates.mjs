 ...
 * read-only by instruction — this prompt is the only (best-effort) read-only
 * lever; there is no hard write-guard (agy also runs under --sandbox, but that
 * fences only the terminal, not file writes).
 *
