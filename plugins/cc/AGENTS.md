...
## 結構角色(判斷,不是清單)
- **無對 commander 的 self-recommend surface,而且現在沒有替代品** —— cc 沒有 subagent,
  fleet 的 `delegating-to-fleet` 路由索引也已隨 fleet plugin 一起移除(見 `docs/adr/0001`
  的 superseded 註記)。commander 只有在**使用者明講**時才會走到 `/cc:task`;它不會自己
  想到 cc。要恢復自動發現,唯一的辦法是給 cc 一顆 proactive subagent(如 codex / agy 那樣)。
  (`skills/cc-handoff` 是給 **Codex host** 的交棒入口,不是給 commander 的。)
...
- cc 會 spawn 完整 Claude Code,可能 re-trigger 委派;`CLAUDE_CC_ACTIVE` 設進 engine env 擋遞迴,
  「keep chains flat」仍是設計指引(原本寫在 fleet 的路由索引裡,那顆已移除)。
